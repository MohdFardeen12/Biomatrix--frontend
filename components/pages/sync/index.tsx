...
//                 <TableHead className="h-11 px-4 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
...
//                 </TableHead>
//                 <TableHead className="h-11 px-4 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-600">
...
//                 </TableHead>
//                 <TableHead className="h-11 px-4 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-600">Enrolled/Total </TableHead>
//                 <TableHead className="h-11 px-4 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-600">Sync Button</TableHead>
//               </TableRow>
//             </TableHeader>
...
//                     <TableCell className="text-[13px] font-semibold font-mono text-slate-700">{record.machineId}</TableCell>
//                     <TableCell className="text-slate-700 font-medium text-[12px]">
//                       {/* <div className="flex items-center gap-2">
//                         <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 text-xs font-bold text-white">
...
//                       {record.operatorName}
//                     </TableCell>
//                     <TableCell className=" font-medium text-[12px] text-slate-700">{record.centerId}</TableCell>
//                     <TableCell className="text-center">
//                       <div className="flex flex-col items-center gap-1">
//                         <span className="text-[12px] font-medium text-slate-700">{formatDate(date)}</span>
//                         <span className="font-mono text-[12px] text-slate-700">{lastSyncTime}</span>
//                       </div>
//                     </TableCell>
...
//           {/* Pagination */}
//           {totalPages > 1 && (
//             <div className="flex items-center justify-between border-t bg-slate-50/50 border-slate-200 px-4 py-3">
//               <span className="text-xs text-slate-600">Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, sorted.length)} of {sorted.length} records</span>
//               <div className="flex items-center gap-1">
...
interface AttendanceRecord {
  ...
  centerSync: {
    date: string;
    lastSyncTime: string;
    enrolledByMachine: number; // students enrolled by this machine
    totalInCenter: number;     // total students in center
    status: SyncStatus;
  };
}
...
type FilterFieldKey = "machineId" | "operatorName" | "centerId" | "syncStatus";
...
const FILTER_FIELDS: FilterField[] = [
  ...
  { key: "operatorName", label: "Operator Name" },
  { key: "centerId",     label: "Center ID"     },
  // NEW: Added sync status filter field
  { key: "syncStatus",   label: "Sync Status"   },
];

interface TextFilter { value: string }

// NEW: Added date range filter structure
...
interface ActiveFilters {
  ...
  // NEW: Added date filter in active filters
  date: DateRangeFilter;
}

const emptyFilters = (): ActiveFilters => ({
  text: {},
  // NEW: Default empty date filter values
  date: { from: "", to: "" },
});
...
function countActiveFilters(f: ActiveFilters) {
  ...
  // NEW: Count date filter if either from or to is selected
  const dateCount = f.date.from || f.date.to ? 1 : 0;

  return textCount + dateCount;
}

// ─── Static Data ──────────────────────────────────────────────────────────────

const initialData: AttendanceRecord[] = [
  { id: "1",  machineId: "MCH-001", operatorName: "Ravi Kumar",    centerId: "DOME-DELHI-042",   centerSync: { date: "2026-03-09", lastSyncTime: "09:45 AM", enrolledByMachine: 98,  totalInCenter: 210, status: "synced"  } },
  { id: "2",  machineId: "MCH-002", operatorName: "Priya Sharma",  centerId: "DOME-MUMBAI-017",  centerSync: { date: "2026-03-09", lastSyncTime: "11:20 AM", enrolledByMachine: 74,  totalInCenter: 185, status: "pending" } },
  { id: "3",  machineId: "MCH-003", operatorName: "Amit Verma",    centerId: "DOME-BLORE-089",   centerSync: { date: "2026-03-08", lastSyncTime: "06:15 PM", enrolledByMachine: 0,   totalInCenter: 162, status: "error"   } },
  { id: "4",  machineId: "MCH-004", operatorName: "Sunita Patel",  centerId: "DOME-HYD-031",     centerSync: { date: "2026-03-09", lastSyncTime: "08:30 AM", enrolledByMachine: 143, totalInCenter: 143, status: "synced"  } },
  { id: "5",  machineId: "MCH-005", operatorName: "Deepak Nair",   centerId: "DOME-CHENNAI-056", centerSync: { date: "2026-03-07", lastSyncTime: "03:55 PM", enrolledByMachine: 51,  totalInCenter: 197, status: "pending" } },
  { id: "6",  machineId: "MCH-006", operatorName: "Meena Joshi",   centerId: "DOME-PUNE-073",    centerSync: { date: "2026-03-09", lastSyncTime: "10:10 AM", enrolledByMachine: 119, totalInCenter: 119, status: "synced"  } },
  { id: "7",  machineId: "MCH-007", operatorName: "Karan Singh",   centerId: "DOME-JAIPUR-012",  centerSync: { date: "2026-03-08", lastSyncTime: "02:30 PM", enrolledByMachine: 88,  totalInCenter: 175, status: "pending" } },
  { id: "8",  machineId: "MCH-008", operatorName: "Anjali Rao",    centerId: "DOME-KOLKATA-034", centerSync: { date: "2026-03-09", lastSyncTime: "07:50 AM", enrolledByMachine: 0,   totalInCenter: 204, status: "error"   } },
  { id: "9",  machineId: "MCH-009", operatorName: "Vikram Menon",  centerId: "DOME-NAGPUR-061",  centerSync: { date: "2026-03-09", lastSyncTime: "12:05 PM", enrolledByMachine: 132, totalInCenter: 132, status: "synced"  } },
  { id: "10", machineId: "MCH-010", operatorName: "Pooja Gupta",   centerId: "DOME-LUCKNOW-048", centerSync: { date: "2026-03-07", lastSyncTime: "04:40 PM", enrolledByMachine: 63,  totalInCenter: 189, status: "pending" } },
  { id: "11", machineId: "MCH-011", operatorName: "Rahul Desai",   centerId: "DOME-SURAT-029",   centerSync: { date: "2026-03-09", lastSyncTime: "09:15 AM", enrolledByMachine: 107, totalInCenter: 107, status: "synced"  } },
  { id: "12", machineId: "MCH-012", operatorName: "Kavya Nair",    centerId: "DOME-KOCHI-077",   centerSync: { date: "2026-03-08", lastSyncTime: "05:25 PM", enrolledByMachine: 39,  totalInCenter: 158, status: "pending" } },
  { id: "13", machineId: "MCH-013", operatorName: "Suresh Yadav",  centerId: "DOME-BHOPAL-053",  centerSync: { date: "2026-03-09", lastSyncTime: "11:45 AM", enrolledByMachine: 0,   totalInCenter: 141, status: "error"   } },
  { id: "14", machineId: "MCH-014", operatorName: "Nisha Iyer",    centerId: "DOME-INDORE-066",  centerSync: { date: "2026-03-09", lastSyncTime: "08:00 AM", enrolledByMachine: 96,  totalInCenter: 96,  status: "synced"  } },
  { id: "15", machineId: "MCH-015", operatorName: "Arjun Tiwari",  centerId: "DOME-PATNA-038",   centerSync: { date: "2026-03-07", lastSyncTime: "01:30 PM", enrolledByMachine: 57,  totalInCenter: 223, status: "pending" } },
  { id: "16", machineId: "MCH-016", operatorName: "Divya Reddy",   centerId: "DOME-VIZAG-081",   centerSync: { date: "2026-03-09", lastSyncTime: "10:55 AM", enrolledByMachine: 174, totalInCenter: 174, status: "synced"  } },
  { id: "17", machineId: "MCH-017", operatorName: "Manish Tomar",  centerId: "DOME-AGRA-022",    centerSync: { date: "2026-03-08", lastSyncTime: "03:10 PM", enrolledByMachine: 45,  totalInCenter: 167, status: "pending" } },
  { id: "18", machineId: "MCH-018", operatorName: "Shreya Pillai", centerId: "DOME-TRICHY-044",  centerSync: { date: "2026-03-09", lastSyncTime: "07:20 AM", enrolledByMachine: 0,   totalInCenter: 138, status: "error"   } },
];
...
function EnrolledProgress({ enrolledByMachine, totalInCenter }: { enrolledByMachine: number; totalInCenter: number }) {
  const pct = totalInCenter === 0 ? 0 : Math.round((enrolledByMachine / totalInCenter) * 100);
  return (
    <div className="flex min-w-[140px] flex-col gap-1.5">
      <div className="flex items-center justify-between text-xs font-medium">
        <span className="text-slate-700">
          {enrolledByMachine.toLocaleString()}
          <span className="font-normal text-slate-400"> / {totalInCenter.toLocaleString()}</span>
        </span>
        <span className={`font-semibold ${pct === 100 ? "text-emerald-600" : pct > 50 ? "text-amber-600" : "text-red-500"}`}>
          {pct}%
        </span>
      </div>
      <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
        <div
          className={`h-full rounded-full transition-all duration-700 ${pct === 100 ? "bg-emerald-500" : pct > 50 ? "bg-amber-400" : "bg-red-400"}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
function SidebarFilter({ open, onClose, filters, onApply, onClearAll }: {
  ...
  const [draft, setDraft] = useState<ActiveFilters>(() => JSON.parse(JSON.stringify(filters)));
  const [expanded, setExpanded] = useState<Partial<Record<FilterFieldKey | "date", boolean>>>({});
  const ref = useRef<HTMLDivElement>(null);
  ...
  const toggle = (key: FilterFieldKey | "date") => setExpanded((p) => ({ ...p, [key]: !p[key] }));
  const setText = (key: FilterFieldKey, value: string) => setDraft((p) => ({ ...p, text: { ...p.text, [key]: { value } } }));

  // NEW: Date range input handlers
      ...
      <div ref={ref} className={`fixed top-0 right-0 z-40 h-full w-80 bg-white border-l border-slate-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          ...
          {(() => {
            ...
            const hasValue = draft.date.from.trim() !== "" || draft.date.to.trim() !== "";
              ...
              <div className="border-b border-slate-100">
                ...
                {isOpen && (
                  <div className="px-5 pb-4 pt-1 space-y-3">
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-1 uppercase tracking-wide">From Date</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-1 uppercase tracking-wide">To Date</label>
                      <input
                        type="date"
                        value={draft.date.to}
...
export default function AttendanceSync() {
  ...
  const filtered = useMemo(() => {
    ...
    for (const [key, f] of Object.entries(activeFilters.text) as [FilterFieldKey, TextFilter][]) {
      ...
      // NEW: Handle sync status filter separately from normal text fields
      if (key === "syncStatus") {
        data = data.filter((r) => statusConfig[r.centerSync.status].label.toLowerCase().includes(q) || r.centerSync.status.toLowerCase().includes(q));
        continue;
      }
    ...
    if (activeFilters.date.from || activeFilters.date.to) {
      data = data.filter((r) => {
        const recordDate = new Date(`${r.centerSync.date}T00:00:00`);

        if (activeFilters.date.from) {
          const fromDate = new Date(`${activeFilters.date.from}T00:00:00`);
          if (recordDate < fromDate) return false;
        }

        if (activeFilters.date.to) {
          const toDate = new Date(`${activeFilters.date.to}T23:59:59`);
    ...
    setTimeout(() => {
      setRecords((prev) => prev.map((r) => r.id === id ? {
        ...r, centerSync: {
          ...
          date: new Date().toISOString().split("T")[0],
          lastSyncTime: new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true }),
          enrolledByMachine: r.centerSync.totalInCenter,
          status: "synced",
        }
      } : r));
    }, 1800);
  ...
  const syncedCount  = records.filter((r) => r.centerSync.status === "synced").length;
  const pendingCount = records.filter((r) => r.centerSync.status === "pending").length;
  const errorCount   = records.filter((r) => r.centerSync.status === "error").length;
  const syncingCount = records.filter((r) => r.centerSync.status === "syncing").length;

  const totalEnrolled   = records.reduce((s, r) => s + r.centerSync.enrolledByMachine, 0);
  const totalInCenters  = records.reduce((s, r) => s + r.centerSync.totalInCenter, 0);
  const overallPct      = totalInCenters === 0 ? 0 : Math.round((totalEnrolled / totalInCenters) * 100);

  // ── Export ─────────────────────────────────────────────────────────────────

  const exportToCSV = () => {
    const headers = ["Machine ID","Operator Name","Center ID","Date & Time","Enrolled by Machine","Total in Center","% Enrolled","Status"];
    const rows = records.map((r) => {
      const pct = r.centerSync.totalInCenter === 0 ? 0 : Math.round((r.centerSync.enrolledByMachine / r.centerSync.totalInCenter) * 100);
      return [r.machineId, r.operatorName, r.centerId, formatDateTime(r.centerSync.date, r.centerSync.lastSyncTime), r.centerSync.enrolledByMachine, r.centerSync.totalInCenter, `${pct}%`, statusConfig[r.centerSync.status].label];
    });
    const csv = [headers.join(","), ...rows.map((r) => r.map((c) => `"${c}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
  ...
  const exportToPDF = () => {
    ...
    const rowsHtml = records.map((r) => {
      const pct = r.centerSync.totalInCenter === 0 ? 0 : Math.round((r.centerSync.enrolledByMachine / r.centerSync.totalInCenter) * 100);
      return `<tr><td>${r.machineId}</td><td>${r.operatorName}</td><td>${r.centerId}</td><td>${formatDateTime(r.centerSync.date, r.centerSync.lastSyncTime)}</td><td>${r.centerSync.enrolledByMachine} / ${r.centerSync.totalInCenter} (${pct}%)</td><td>${statusConfig[r.centerSync.status].label}</td></tr>`;
    }).join("");
    printWindow.document.write(`<html><head><title>Attendance Sync Report</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#0f172a}h1{font-size:20px;margin-bottom:8px}p{font-size:12px;color:#64748b;margin-bottom:16px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #e2e8f0;padding:8px;text-align:left}th{background:#f8fafc;color:#475569;text-transform:uppercase;font-size:11px;letter-spacing:.04em}</style></head><body><h1>Attendance Sync Report</h1><p>Generated on ${new Date().toLocaleString("en-IN")}</p><table><thead><tr><th>Machine ID</th><th>Operator Name</th><th>Center ID</th><th>Date & Time</th><th>Enrolled / Total</th><th>Status</th></tr></thead><tbody>${rowsHtml}</tbody></table></body></html>`);
    printWindow.document.close(); printWindow.focus(); printWindow.print();
  };
      ...
      <div className="px-2 max-w-8xl">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="ml-2">
            ...
            <p className="mt-1 text-sm text-slate-500">{syncedCount} of {records.length} machines synced</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="text-xs font-medium text-slate-500">Total Enrolled Across All Centers:</span>
              <span className="text-sm font-semibold text-slate-800">
                {totalEnrolled.toLocaleString()}
                <span className="text-xs font-normal text-slate-400"> / {totalInCenters.toLocaleString()}</span>
              </span>
              <span className={`rounded-md px-2 py-0.5 text-xs font-semibold ${overallPct === 100 ? "bg-emerald-100 text-emerald-700" : overallPct >= 70 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-600"}`}>
                {overallPct}%
              </span>
            </div>
            <div className="mt-2 h-2 w-64 overflow-hidden rounded-full bg-slate-100">
              <div
                className={`h-full rounded-full transition-all duration-700 ${overallPct === 100 ? "bg-emerald-500" : overallPct >= 70 ? "bg-amber-400" : "bg-red-400"}`}
                style={{ width: `${overallPct}%` }}
              />
            </div>
          </div>
        ...
        {filterBadgeCount > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {/* NEW: Date filter chip */}
            {(activeFilters.date.from || activeFilters.date.to) && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-[11px] text-slate-600 font-medium">
                Date: {activeFilters.date.from || "Any"} → {activeFilters.date.to || "Any"}
                <button
                  onClick={() => {
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <Table>
              ...
              <TableRow className="bg-slate-50 hover:bg-slate-50">
                <TableHead className="h-11 px-4 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  <div className="inline-flex items-center gap-1">
                    Machine ID <SortIconButton columnKey="machineId" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} isText />
                  </div>
                </TableHead>
                <TableHead className="h-11 px-4 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  <div className="inline-flex items-center gap-1">
                    Operator Name <SortIconButton columnKey="operatorName" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} isText />
                  </div>
                </TableHead>
                <TableHead className="h-11 px-4 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  <div className="inline-flex items-center gap-1">
                    Center ID <SortIconButton columnKey="centerId" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} isText />
                  </div>
                </TableHead>
                <TableHead className="h-11 px-4 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  <div className="inline-flex items-center justify-center gap-1">
                    Date & Time <SortIconButton columnKey="date" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                  </div>
                </TableHead>
                <TableHead className="h-11 px-4 text-center text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  Enrolled / Total
            ...
            <TableBody>
              {paginatedRecords.map((record) => {
                const { date, lastSyncTime, enrolledByMachine, totalInCenter, status } = record.centerSync;
                const cfg = statusConfig[status];
                const isSyncing = status === "syncing";
                return (
                  <TableRow key={record.id} className="border-b border-slate-200 transition-colors hover:bg-slate-50/80">
                    <TableCell className="text-[13px] font-semibold font-mono text-slate-700">{record.machineId}</TableCell>
                    <TableCell className="text-slate-700 font-medium text-[12px]">{record.operatorName}</TableCell>
                    <TableCell className="font-medium text-[12px] text-slate-700">{record.centerId}</TableCell>
                    ...
                    {/* Enrolled / Total with progress */}
                    <TableCell>
                      <div className="flex justify-center">
                        <EnrolledProgress enrolledByMachine={enrolledByMachine} totalInCenter={totalInCenter} />
                      </div>
                    </TableCell>
                    {/* Sync Status badge */}
                    <TableCell className="text-center">
                      <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium ${cfg.color}`}>
                        {cfg.icon}{cfg.label}
                      </span>
                    </TableCell>
                    {/* Re-sync button */}
                    <TableCell className="text-center">
                      <Button
                        ...
                        className={`gap-1.5 text-xs font-medium transition-all ${
                          isSyncing
                            ? "cursor-not-allowed border-blue-300 text-blue-600"
                            : "border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-blue-400 hover:text-blue-600"
                        }`}
                      >
                        <RefreshCw className={`h-3 w-3 ${isSyncing ? "animate-spin" : ""}`} />

...
import { ChevronDown, Search, SlidersHorizontal, X } from "lucide-react" // ✅ ADDED X icon
import { NavUser } from "./nav-user"
import { BreadcrumbLinkDemo } from "./Breadcrumb"
...
export function SiteHeader() {
  ...
  // ✅ ADDED: track selected filter (City / Center)
    ...
    <header className="flex flex-col shrink-0 border-b transition-[width,height] ease-linear">
      ...
      <div className="flex w-full flex-wrap items-center gap-2 px-3 pt-2 pb-2 lg:px-6">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          {/* ✅ WRAPPED button to allow absolute positioning */}
          <div className="relative">
            ...
            {/* ✅ ADDED: Corner cross button to reset filter */}
            {selectedFilter !== "Filters" && (
              <button
                onClick={() => setSelectedFilter("Filters")}
                className="absolute -top-1 -right-1 bg-white border rounded-full p-[2px] shadow-sm hover:bg-gray-100"
              >
                <X className="h-3 w-3" />
              </button>
            )}
